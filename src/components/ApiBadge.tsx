...
const freeTierLabelsAr: Record<FreeTierType, string> = {
  "free-tier": "مجاني",
...
const difficultyLabelsAr: Record<Difficulty, string> = {
  easy: "مبتدئ",
  medium: "متوسط",
  advanced: "متقدم",
};
...
export default function ApiBadge({ variant, value, locale = "en" }: Props) {
  if (variant === "freeTier") {
    ...
    const label = locale === "ar" ? freeTierLabelsAr[v] : getFreeTierLabel(v);
    return (
  ...
  const label = locale === "ar" ? difficultyLabelsAr[v] : getDifficultyLabel(v);
  return (
    <span className={`inline-flex items-center px-2 py-0.5 rounded-md text-xs font-semibold ${colorMap[v]}`}>
      {label}
    </span>
