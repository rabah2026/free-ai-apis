...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!guide) return {};
  return {
    title: `${guide.titleAr ?? guide.title} | مرشد واجهات الذكاء الاصطناعي`,
    description: guide.summaryAr ?? guide.summary,
  };
}

export default function ArGuideDetailPage({ params }: Props) {
  const guide = guides.find((g) => g.slug === params.slug);
  if (!guide) notFound();
  ...
  return (
    <div dir="rtl" className="max-w-2xl mx-auto px-4 sm:px-6 py-12">
      <Link href="/ar/guides" className="text-sm text-indigo-400 hover:underline mb-8 inline-block">
        ← العودة إلى الأدلة
      </Link>
      <div className="text-xs text-[#4a6080] mb-3">
        {guide.readingTime} قراءة · آخر تحقق {guide.lastVerified}
      </div>
      ...
      <div dir="ltr" className="space-y-5 text-left">
        {paragraphs.map((para, i) => {
          ...
          if (para.startsWith("```")) {
            ...
            return (
              <pre key={i} className="bg-[#0a0d14] border border-[#1e2d45] rounded-xl p-4 overflow-x-auto text-sm text-[#c8d6ef] font-mono">
                {code}
              </pre>
            );
          }
          if (para.match(/^`[^`]+`$/)) {
            return (
              <pre key={i} className="bg-[#0a0d14] border border-[#1e2d45] rounded-xl p-4 text-sm text-[#c8d6ef] font-mono">
                {para.replace(/`/g, "")}
              </pre>
            );
          }
          ...
          return (
            <p
              key={i}
              className="text-[#7d94b5] leading-relaxed text-sm"
